...
import { errorHandler, ResponseError } from "~/src/infrastructure/response/response.ts"
import { UserRepositoryPg, UserService, UserRouterHono } from "~/src/service/user/user.ts"
import { ShortenerRepositoryPg, ShortenerClickRepositoryPg, ShortenerService, ShortenerRouterHono } from "~/src/service/shortener/shortener.ts"
...
export const shortenerService = new ShortenerService(repository.shortenerRepository, repository.shortenerClickRepository)
...
app.use("*", async (c, next) => {
	...
	const token = c.req.headers.get("Authorization")?.slice(7) || cookies.session_token
	if (!token) {
		return next()
	}
